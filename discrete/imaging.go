...
func EncodeSize(name string, dx,dy int,bs ...[]uint8) error {
	if len(bs) < 1 {
		return fmt.Errorf("Nothing to encode.")
	}
	...
	img:=transform.Resize(
		Image8bit(blueOverGrey, bs[0], bs[1:]...),
		dx,
		dy,
		transform.Linear,
	)
	// draw centre line
	(&composable{img}).drawOverAt(image.Rectangle{Max: image.Pt(dx, 2)}, image.Pt(0, dy/2))
...
func Image8bit(s Appearance, bs0 []uint8, bss ...[]uint8) *composable {
...
func Resize(img image.Image, dx,dy uint) *image.NRGBA {
...
func (r resized) At(x,y int) color.Color{
	return r.Image.At(int(float32(x)*xd),int(float32(y)*yd))
}

// procedural excess size intermediate image
