// package containing oscillators driven equally
package group
...
// import "../../stepped" // actually as above, just not obviously so.
...
func ExampleDriverGroupImage() {
	...
	o:=Steppers{
		oscillator.NewDrivenAndDamped(steps/2,.85),
//		oscillator.NewDrivenAndDamped(steps/2,.5),
//		oscillator.NewDrivenAndDamped(steps/4,.5),
		oscillator.NewDrivenAndDamped(steps/32,.05),
	}
	o.Step(127) // initial drive, never exceeded, so this will always fit byte
	
	var r,a []byte
	for r=make([]byte,0,steps*cycles);len(r)!=cap(r);o.Step(0){
		r=append(r,byte(o.Real()/float64(len(o))+128))
		a=append(a,byte(o.Abs()+128))
	}
	fmt.Println(encode(fmt.Sprintf("%v.png", len(r)),r,a))
	// Output:
	// <nil>
}
